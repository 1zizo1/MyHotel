...
export default function Search() {
    const [isOpen, setIsOpen] = useState<boolean>(true); // State to manage dropdown visibility
    ...
    const handleHotelTypeChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setSelectedHotelTypes((prevHotelType) => event.target.checked
 ...
 <div className="rounded-lg border border-slate-300 p-5 h-fit">
      <div className="flex justify-between items-center cursor-pointer" onClick={toggleDropdown}>
        ...
        <button className="text-blue-600">
          {isOpen ? <BsArrowDownCircle size={24} className="text-slate-300 transition-transform transform hover:scale-105 hover:text-blue-500"/> : <BsArrowUpCircle size={24} className="text-slate-300 transition-transform transform hover:scale-105 hover:text-blue-500"/>}
        </button>
      </div>
      {isOpen && (
